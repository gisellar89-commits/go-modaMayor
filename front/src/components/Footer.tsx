          ...
          {addresses.length > 0 && (
            <div className="space-y-3 text-sm">
              {addresses.slice(0, 2).map((addr, index) => (
                <div key={addr.id || `addr-${index}`} className="space-y-1">
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <span className="font-semibold">{addr.address}</span>
                  </div>
                  {addr.business_hours && (
                    <div className="flex items-center justify-center gap-2">
                      <svg className="w-5 h-5 text-pink-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="hidden md:grid grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div>
            ...
            {addresses.length > 0 && (
              <div className="space-y-2 text-sm">
                {addresses.slice(0, 2).map((addr, index) => (
                  <div key={addr.id || `addr-${index}`} className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      {addr.business_hours && (
                        <p className="text-xs text-pink-300 mt-0.5">🕐 {addr.business_hours}</p>
                      )}
                    </div>
                  </div>
                ))}
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              ...
              <li>
                <Link href="/productos" className="hover:text-yellow-400 transition-colors flex items-center gap-1">
                  <span>→</span> Todos los Productos
                </Link>
              </li>
              <li>
                <Link href="/productos?filter=liquidacion" className="hover:text-yellow-400 transition-colors flex items-center gap-1">
                  <span>→</span> ⚡ Liquidación
                </Link>
              </li>
              <li>
                <Link href="/contacto" className="hover:text-yellow-400 transition-colors flex items-center gap-1">
                  <span>→</span> Contacto
                </Link>
              </li>
              <li>
            ...
            <div className="space-y-3 text-sm mb-4">
              {contactSettings.whatsapp_number && (
                ...
                >
                  ...
                  <span>WhatsApp: {contactSettings.phone || contactSettings.whatsapp_number}</span>
                </a>
              )}
