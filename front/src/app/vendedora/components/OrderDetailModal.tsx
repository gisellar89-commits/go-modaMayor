...
export default function OrderDetailModal({ open, order, onClose }: Props) {
  ...
  const rawStatus = (o.status ?? o.Status ?? "").toString();
  ...
  const mapStatus = (s: string) => {
    ...
    if (low.includes("completado") || low.includes("finaliz") || low.includes("entreg")) return "Completado";
    if (low.includes("pagado")) return "Pagado";
    if (low.includes("en camino") || low.includes("en_camino") || low.includes("procesando")) return "En camino";
    if (low.includes("asign")) return "Asignada";
    if (low.includes("esper") || low.includes("vended") || low.includes("pendiente")) return "Pendiente";
    return low.split(/[_\s-]+/).map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
  };

  const status = mapStatus(rawStatus);
  let statusColor = "bg-gray-100 text-gray-800";
  if (status === "Completado") statusColor = "bg-green-100 text-green-800";
  else if (status === "Pagado") statusColor = "bg-blue-100 text-blue-800";
  else if (status === "Asignada") statusColor = "bg-purple-100 text-purple-800";
  else if (status === "Pendiente") statusColor = "bg-yellow-100 text-yellow-800";
  else if (status === "Cancelado") statusColor = "bg-red-100 text-red-800";

  const userName = o.User?.Name || o.User?.name || o.User?.email || `Usuario #${o.user_id || o.UserID}`;
        ...
        <div className="bg-white rounded-lg shadow-xl transform transition-all duration-200 ease-out scale-100">
          <div className="px-6 py-4 border-b flex items-start justify-between">
            <div>
              ...
              <div className="text-sm text-gray-600 mt-1">
                <span className="font-medium">Vendedora:</span> vendedor1
              </div>
            </div>
          ...
          <div className="px-6 py-4">
            <div className="mb-3 text-sm text-gray-600">
              Fecha: {new Date(o.created_at ?? o.createdAt ?? o.CreatedAt ?? Date.now()).toLocaleString()}
            </div>
                    ...
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-medium">
                          {it.product_name || it.product?.name ?? it.product?.title ?? it.Product?.name ?? 'Producto'}
                        </div>
                        {variantText !== 'N/A' && (
